...
first of the 5 mandatory stages every story passes through in strict order - READY, IMPLEMENTED,
REVIEWED, TESTED, ACCEPTED - via `advance_story_stage(title_or_id, stage)`. That tool mechanically
rejects marking a story READY if the checks below aren't met (missing/placeholder title, blank or
still-templated user story, no acceptance criteria) - this checklist is what to fix if it does.
