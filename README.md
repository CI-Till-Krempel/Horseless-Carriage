...
- `setup_all.py` — guided, orchestrated setup: asks about developer mode first (before any container work happens), then runs `setup_llm.py`, then `setup_project.py`, then gates on `doctor.py` (looping fix→retry until there are no more ERROR-level items), then offers to start the agent via `run.py`. Each step below is still a fully standalone script this just chains for a first-time/new-machine setup.
