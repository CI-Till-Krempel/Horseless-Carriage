...
def create_sprint_report(summary: str, accomplishments: List[str], tool_context=None) -> Dict[str, Any]:
    ...
    # ISSUE-0001: closing this sprint's report "uses up" its human sprint
    # approval - the next sprint's stories can't reach Implemented again
    # until a fresh one is recorded via record_human_approval("sprint").
    s["sprint_approval_baseline"] = sum(1 for a in s.get("human_approvals", []) if a.get("type") == "sprint")
