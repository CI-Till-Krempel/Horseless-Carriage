...
def read_doc(path: str, tool_context=None) -> Dict[str, Any]:
    """
    Read any file within the /docs folder of the repository.
    """
    repo_root = _configured_repo_root(tool_context)
    ...
    if not clean_path.startswith("docs/"):
        return {"status": "error", "message": f"Access denied. Path '{path}' must be within the 'docs/' directory."}
        
    full_path = (repo_root / clean_path).resolve()
    
    # Safety: ensure the path is within repo_root/docs
    docs_root = (repo_root / "docs").resolve()
    if not str(full_path).startswith(str(docs_root)):
        return {"status": "error", "message": f"Path '{path}' is outside the 'docs/' directory."}
...
def upsert_prd(content: str, filename: str, tool_context=None) -> Dict[str, Any]:
    """
    Create or update a Product Requirements Document (PRD) in docs/requirements/.
    ...
    path = f"docs/requirements/{filename}"
...
def upsert_srs(content: str, filename: str, tool_context=None) -> Dict[str, Any]:
    """
    Create or update a Software Requirements Specification (SRS) in docs/requirements/.
