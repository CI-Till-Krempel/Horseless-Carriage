...
def emit_model_entry(role: str, provider: str, model: str, ollama_api_base: str = "http://ollama:11434") -> str:
    ...
    if provider == "gemini":
        ...
        lines += [f"      model: ollama/{model}", f"      api_base: {ollama_api_base}"]
    lines.append("")
    return "\n".join(lines)


def write_litellm_yaml(provider: str, main_model: str, cheap_model: str, out_file: Path,
                        ollama_api_base: str = "http://ollama:11434") -> None:
    parts = [f"# Generated by setup_llm.py - provider: {provider}, main model: {main_model}", "model_list:"]
    for role in ROLES:
        parts.append(emit_model_entry(role, provider, main_model, ollama_api_base))
    parts.append(emit_model_entry("scrum-eval-cheap", provider, cheap_model, ollama_api_base))
...
# --- Host-native Ollama (GH issue #93) ---

def host_ollama_default_enable(is_macos: bool, current_value: str) -> bool:
    """Whether to default the "run Ollama natively on the host?" prompt to
    yes: same precedence rule as gpu_default_enable - an explicit prior
    choice wins over a fresh recommendation. On a first-time configuration,
    default to yes on macOS (a dockerized Ollama there can never use the
    GPU at all - see detect_nvidia_gpu's docstring and docs/SETUP.md's "GPU
    Support" section) and no elsewhere (the dockerized `ollama` service
    already works fine, with optional NVIDIA GPU passthrough via
...
def run_configuration_test(provider: str, model_label: str, env_path: Path, dev: bool = False) -> None:
    ...
    compose_args = []
    extra_service = None
    host_ollama_mode = provider == "local" and lib_env.read_env_var(env_path, "OLLAMA_HOST_MODE") == "true"
    if provider == "local":
        if host_ollama_mode:
            # A wholly separate compose file, not docker-compose.local.yaml
            # plus an overlay - see docker-compose.local-hostollama.yaml's
            # header comment (Compose merges, rather than replaces,
            # `depends_on` across `-f` files).
            compose_args = ["-f", "docker-compose.local-hostollama.yaml"]
        else:
    ...
    if host_ollama_mode:
        ...
        print(f"configured model is pulled (ollama pull {model_label}) - this test talks to it")
        print("directly, no container is started for Ollama itself.")
    ...
    # image about to be discarded. Not applicable in host mode - there's no
...
def run_local_provider(dev: bool = False) -> None:
    ...
    if is_macos:
        print("Docker Desktop for Mac has no GPU passthrough at all (see docs/SETUP.md's \"GPU")
        print("Support\" section) - a containerized Ollama always runs CPU-only here, even on")
        print("Apple Silicon. Running Ollama natively on the host instead lets it use the GPU")
        print("(Metal) directly (GH issue #93).")
    else:
        print("Ollama can run in its own container (simplest - just `docker compose up`), or")
        print("natively on the host machine instead - e.g. if you already have it installed,")
        print("or want this one instance to serve other things too.")
    current_host_mode = lib_env.read_env_var(env_path, "OLLAMA_HOST_MODE")
    default_host_mode = host_ollama_default_enable(is_macos, current_host_mode)
    default_label = "Y/n" if default_host_mode else "y/N"
    host_answer = input(f"Run Ollama natively on the host instead of in a container? [{default_label}]: ").strip().lower()
    host_mode = host_answer.startswith("y") if host_answer else default_host_mode
    if host_mode:
        print(f"Native mode selected - before starting the stack, run `ollama serve` on this")
        print(f"machine and pull the model: ollama pull {model}")
    lib_env.update_env_var(env_path, "OLLAMA_HOST_MODE", "true" if host_mode else "false")

    print()
    print("--- GPU acceleration (dockerized Ollama only) ---")
    if host_mode:
        print("Skipped - Ollama is running natively on the host, not in a container, so this")
        print("dockerized NVIDIA passthrough setting doesn't apply here. Your host's own Ollama")
        print("install uses whatever GPU acceleration it detects on its own.")
        gpu_enabled = False
    ...
    ollama_api_base = "http://host.docker.internal:11434" if host_mode else "http://ollama:11434"
    write_litellm_yaml("local", model, model, Path("config/model-templates/litellm.local-ollama.yaml"), ollama_api_base)

    print()
    print("--- Provider & model ---")
    print("Provider: Local / Ollama")
    print(f"Model (all roles + eval harness): {model}")
    print(f"Ollama runs: {'natively on the host' if host_mode else 'in its own container'}")
