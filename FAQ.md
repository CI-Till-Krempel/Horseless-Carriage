...
via [Ollama](https://ollama.com) (**⚠️ Experimental — not yet thoroughly tested**) — no commercial
API key at all. `setup_llm.py` walks you through picking one. See
[Configuration Reference](docs/CONFIGURATION.md) for how the provider choice interacts with
everything else.
...
calls once the model is pulled. **⚠️ Experimental — not yet thoroughly tested.** See
[Setup § Running fully local](docs/SETUP.md#running-fully-local-no-commercial-llm).
